...
@section('content')
   ...
   <div class="mb-8" x-data>
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
         <div>
            <h2 class="text-xl sm:text-xl font-bold text-gray-900">Package Management</h2>
            <p class="text-gray-600 mt-1 italic">Manage your packages and pricing</p>
         </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm overflow-hidden mt-5">
         ...
         <div class="px-6 py-3 border-b border-gray-200">
            <div class="flex items-center justify-between">
               @can('packages.create')
                  <a href="{{ route('account.packages.create') }}" class="inline-flex text-sm items-center px-3 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors">
                     <x-icons.plus class="mr-1" width="20" height="20" />
                     Add New
                  </a>
               @endcan
               <div class="block relative">
                  <form action="{{ route('account.packages.index') }}">
            ...
            <table class="min-w-full divide-y divide-gray-200">
               ...
               @foreach($packages as $package)
                  <tr>
                     ...
                     <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $package->outlet->name ?? '-' }}</td>
                     <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $package->category_package->name ?? '-' }}</td>
                     ...
                     <td class="px-6 py-4 flex text-center text-sm font-medium">
                        @can('packages.edit')
                           <a href="{{ route('account.packages.edit', $package->id) }}" class="text-primary hover:text-primary/90 mr-4">
                        ...
                        @can('packages.delete')
                           <button
                                   @click="$store.deleteModal.open('{{ route('account.packages.destroy', $package->id) }}')"
