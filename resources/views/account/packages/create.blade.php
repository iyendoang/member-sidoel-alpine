...
@section('content')
   ...
   <div x-data class="mb-8">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
         <div>
            <h2 class="text-xl sm:text-xl font-bold text-gray-900">Create Package</h2>
            <p class="text-gray-600 mt-1 italic">Add a new service package</p>
         </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm overflow-hidden mt-5">
         <div class="px-6 py-6 border-b border-gray-200">
            ...
            <form method="POST" action="{{ route('account.packages.store') }}">
               @csrf
               ...
               <div class="space-y-4">
                  ...
                  <div>
                     ...
                     <select name="category_package_id" class="mt-1 block w-full border border-gray-300 @error('category_package_id') border-red-500 @enderror rounded-lg px-3 py-2 focus:ring-primary focus:border-primary">
                        ...
                        @foreach ($categories as $category)
                           <option value="{{ $category->id }}" {{ old('category_package_id') == $category->id ? 'selected' : '' }}>
                              {{ $category->name }}
                           </option>
                        @endforeach
                     </select>
                  ...
                  <div>
                     ...
                     <input type="text" name="name" value="{{ old('name') }}" placeholder="Package Name" class="mt-1 block w-full border border-gray-300 @error('name') border-red-500 @enderror rounded-lg px-3 py-2 focus:ring-primary focus:border-primary">
                     @error('name')
                     <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                     @enderror
                  </div>
                  ...
                  <div>
                     ...
                     <input type="number" name="price" value="{{ old('price') }}" placeholder="Package Price" class="mt-1 block w-full border border-gray-300 @error('price') border-red-500 @enderror rounded-lg px-3 py-2 focus:ring-primary focus:border-primary">
                     @error('price')
                     <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                     @enderror
                  </div>
