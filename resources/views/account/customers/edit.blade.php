...
@section('content')
   ...
   <div class="mb-8">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
         <div>
            <h2 class="text-xl sm:text-xl font-bold text-gray-900">Edit Customer</h2>
            <p class="text-gray-600 mt-1 italic">Update customer data and outlet</p>
         </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm overflow-hidden mt-5">
         <div class="px-6 py-6 border-b border-gray-200">
            ...
            <form method="POST" action="{{ route('account.customers.update', $customer->id) }}">
               @csrf
               @method('PUT')
               ...
               <div class="space-y-4">
                  ...
                  <div>
                     ...
                     <input type="text" name="office_name" value="{{ old('name', $customer->office_name) }}"
                     ...
                     <div>
                        ...
                        <select name="outlet_id"
                           ...
                           @foreach ($outlets as $outlet)
                              <option value="{{ $outlet->id }}" {{ (old('outlet_id', $customer->outlet_id) == $outlet->id) ? 'selected' : '' }}>
                                 {{ $outlet->name }}
                              </option>
                           @endforeach
                        </select>
               ...
               <div class="mt-6 flex justify-start">
                  <a href="{{ route('account.customers.index') }}" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Cancel</a>
